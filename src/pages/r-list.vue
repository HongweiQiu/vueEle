<template>
  <div class="page-body">
    ...
    <div class="box" >
      <el-table :data="table" max-height="550"  
      ...
      v-loading="loading"  element-loading-text="拼命加载中">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <m-button type="success" size="mini" @click='update(scope.row.id)' :disabled="dis">编辑</m-button>
            <m-button type="danger" size="mini" @click='del(scope.row.id)' >删除</m-button>
             <m-button type="primary" size="mini" @click='authorize(scope.row.id)'>授权</m-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block" style="display:flex;padding-top:1%;">
        ...
        :current-page="currentPage4" :page-sizes="[3,5,20,25]" :page-size="3" :size="20" layout="total, sizes, prev, pager, next, jumper" :total="count" @keyup.left.native="left()">
        </el-pagination>
      </div>
    </div>
  </div>
    ...
    methods: {
      ...
      sindex() {
        ...
        const Url = !this.query ?
          `${this.http.url}role/index?page=${pages}&&num=${nums}` : `${this.http.url}role/index?search=name:${this.query};`;
      ...
      update(id) {
       ...
       this.$router.push({ name: 'roleUpdate', query: { id} }) },
      ...
      del(id) {
        ...
        this.$confirm('此操作将永久删除该信息, 是否继续?', '', {
          ...
          const head = { headers: { "api-token": this.$sign(), "Authorization": this.http.vtoken } };
          this.http.axios.get(`${this.http.url}role/delete?id=${id}`, head).then(res => {
            this.sindex();
      ...
      authorize(id){  
        this.$router.push({ name: 'roleOrder', query: { id} })
