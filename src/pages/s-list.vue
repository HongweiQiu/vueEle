  ...
  <div class="page-body">
    ...
    element-loading-spinner="el-icon-loading">
      ...
      <div class="block" style="display:flex;padding-top:1%;">
        ...
        :current-page="page" :page-sizes="[3,5,20,25]" :page-size="3" :size="20" layout="total, sizes, prev, pager, next, jumper" :total="count">
        </el-pagination>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    sindex() {
      ...
      const abbr = `${this.http.url}station/index?`;
      const Url = !this.query ?
        `${abbr}page=${pages}&&num=${nums}` : `${abbr}search=name:${this.query};`;
    ...
    del(id) {
      this.$confirm('此操作将永久删除该信息, 是否继续?', '', {
        ...
        const head = { headers: { "api-token": this.$sign(), "Authorization": this.http.vtoken } };
        this.http.axios.get(`${this.http.url}station/delete?id=${id}`, head).then(res => {
          this.sindex();
