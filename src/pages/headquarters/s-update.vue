...
export default {
  ...
  methods: {
    ...
    getInfo() {
      ...
      this.$axios.get(this.http.url + 'station/info?id=' + this.$route.query.id).then(res => {
    ...
    onSubmit() {
      ...
      const url='api/station/update'
      if (this.form.pwd == '') {
        this.$api.update(url,params,'index','修改成功');
      } else {
         this.$api.update(url,newparams,'index','修改成功');
      }
    },
    reset() {
